...
class BlockTree: public CHeapObj<mtMetaspace> {
  ...
  struct Node {
    ...
    // Tree node to link blocks together.
    TreeNode _tree_node;
  ...
  struct TreeComparator {
    static int cmp(const size_t a, const TreeNode* b) {
      const size_t node_word_size = Node::cast_to_node(b)->_word_size;

      if (a < node_word_size) { return -1; }
      if (a > node_word_size) { return 1; }
      return 0;
    }
...
public:
  ...
  void add_block(MetaBlock block) {
    ...
    auto cursor = _tree.cursor(word_size);
    if (cursor.found()) {
      add_to_list(n, Node::cast_to_node(cursor.node())); // parent stays null in this case.
    }
    else {
