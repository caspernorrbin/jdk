...
void BlockTree::verify() const {
  ...
  // pointer vadility, and that the tree is balanced and not ill-formed.
  _tree.verify_self();

  MemRangeCounter counter;

  _tree.visit_in_order([&](const TreeNode* tree_node) {
...
void BlockTree::print_tree(outputStream* st) const {
  ...
  _tree.print_on(st,
    [&](const TreeNode *tree_node, outputStream *st, int depth) {
    const Node* n = Node::cast_to_node(tree_node);
